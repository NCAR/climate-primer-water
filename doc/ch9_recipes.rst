...
-  Climate – It is sometimes said, "Climate is what you expect,
   weather is what you get." **Climate** is the statistical
   representation of weather over days, months, seasons, years, decades
   and longer.

-  Weather – describes the details of what we experience over the
...
-  Projections – Climate models do predict specific weather
...
-  Initial values – To distinguish weather predictions versus
...
-  Boundary values and climate forcings – With climate models,
   ...
   possible outcomes in a system. Boundary values, sometimes
...
-  Anthropogenic – Natural boundary values, or climate forcings
...
-  ESM – "coupled Earth System Model" or "coupled climate model", where
   ...
   Note the previously-used term “coupled atmosphere-ocean
...
In this STEP2, we will process coarse-scale ESM projection data
...
In this graphic of December–January–February precipitation, the
...
Requirements for local assessments of hydrology and crop water
demands:

-  Model biases removed

-  Short time steps

-  High spatial resolution

Local assessments of hydrology and crop irrigation requirements
...
There are trade-offs in confidence when global model output is
...
Although ESMs produce high temporal resolution output, such as
...
Considerations for time resolution choice

-  Errors in future simulations greater for shorter time steps

-  Balance the need for high resolution vs. sensitivity to
   uncertainty

-  Monthly time steps can be used to generate seasonal cycles

The choice of time step is typically a balance between the
...
Addressing spatial resolution needs for regional studies requires
...
For the various models that contribute to AR4, the spatial
...
-  Removal of consistent, systematic error

Downscaling:

-  Accounting for local controls on regional climate

-  Reproduction of global projections on a finer spatial scale

Bias correction and downscaling are two issues typically
...
Here we are demonstrating bias correction of model data using
hypothetical data.

|image7|

Errors occur in ESMs as is the case with most models. Some errors
...
Other errors have consistency in their patterns or recurrence.
...
**Bias Correction: Historical Period**
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

|image9|

In reality, model bias isn't typically corrected by one constant
...
So we need to quantify the bias correction across a range of
...
Let's see how that works by first ranking the 20 simulated values
...
Likewise, a simulated value in the middle of the observed
...
The monthly precipitation value of 4.5 would be associated with a
cumulative probability of 0.20.

This means that there is only a 20% chance that 4.5 won't be
exceeded. Thus 4.5 is in the lower 20% of the distribution of monthly
values.

|image11|

The red CDF represents values in a historical simulation. So the
...
When the bias correction is applied, the CDF based on simulated
values will take on the shape of the observation-based CDF.

When we look at these two CDF plots together we see that the
...
This is consistent with the time series view that shows a more
...
The amount of adjustment needed to match the simulation based CDF
to the observation-based CDF provides us with a bias correction factor
for each probability.

**Bias Correction: Future Projections**
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Bias-correction of future projections:

1. Assume same model bias for future simulations as for
   historical

2. Remove model bias in future projections without changing
   future trends

For future simulations, we will assume that the model has the
...
There are four types of bias corrections for simulated data:

-  Overall shift to higher values

-  Overall shift to lower values

-  Expand the range of values so that higher values get higher
   and lower values get lower

-  Compress the range of values so that higher values get lower
   and lower values get higher

|image14|

In the example we have been considering, the bias correction is
expanding the range of values.

|image15|

Now what if the same model projection for a future simulation has
...
We apply the bias correction factors derived from the historical
period and we get a bias-corrected future simulation that shows an
expanded range of values.

In the corresponding bias adjusted future time series, the values
have a larger range with even higher high values and lower low values.

|image16|

There are a variety of biases that can show up in model
...
Although there are different implementations, the widely accepted
...
Important assumptions we make when using bias-corrected climate
projection data include:

-  There are no random errors in the climate projection data.

   -  In many cases the projection data will contain both
      systematic error (bias) and random error

-  The most appropriate emissions scenario was chosen for the
   projection data.

   -  The model projection data will only be as good as the model
      and its input data, such as the emissions scenarios.

-  There is stationarity in the physical processes affecting
   climate (the most important of the three)

   -  The assumption is that the current influences on climate
   ...
   -  There are limitations to assuming that the historical base
...
-  Advantages
   ...
   -  Statistically relates coarse resolution data to high
      resolution observed climate

   -  Computationally efficient, easier to generate scenario
      ensembles

-  Limitations

   -  Assumes stationarity in relationship between predictor and
      predictand
...
-  Advantages
   ...
   -  High resolution Regional Climate Model (RCM) to simulate
      regional climate

   -  Doesn’t depend on historical observations

   -  Ideally accounts for complex, non-stationary relationship
...
-  Limitations
   ...
   -  Affected by errors in underlying ESM that provides boundary
      conditions

   -  Different RCMs give different results

There are many spatial downscaling methods, but they can
generally be classified into two categories: 1) statistical, and 2)
dynamical.

Statistical methods start with the coarse resolution ESM
...
Dynamical methods embed a high resolution Regional Climate Model
(RCM) within an ESM to simulate climate processes at spatial scales
finer than that of the driving ESM.

This provides the ability to better capture the effects of
...
Consider this hypothetical situation where we have a coastal
...
In a high-resolution representation, we have a 6 X 6 grid.

|image20|

A 2 X 2 grid represents the coarser ESM resolution.

|image21|

This 2 X 2 figure shows a bias-corrected 30-year mean monthly
...
The first step is to get an observation-based monthly climatology
with high spatial resolution for that same month.

|image22|\ |image23|

Here is the 6 X 6 grid of observation-based monthly climatology
values over the historical period for the same month as the future
simulation.

|image24|\ |image25|

In the next step we need to develop a common spatial resolution
...
e.g. For top-left quadrant: 36 ÷ 40 = 0.9

The next step is to quantify the relationship between the
...
Next we want to map those conversion factors from the coarse grid
...
In the final step, high-resolution observed precipitation
...
We developed this example with exactly the same area-averaged
...
-  Applied to projections produced by the ESMs in IPCC Assessment
   Reports

Although there are reasons for selecting one bias correction
...
The Bias-Correction and Spatial Downscaling methodology, or BCSD,
has been applied to climate projections produced by the AR models.

BCSD methodology:

-  Uses AR data from a variety of spatial resolutions

-  AR data re-gridded to common spatial resolution

-  Statistically downscales the common resolution to a higher
   spatial resolution

The ESM data from the Assessment Reports are at a variety of
...
1. Define a historical period: 1950–1999

2. Run climate projections for historical period and identify
   simulation bias

3. Correct for model simulation bias in historical period

4. Apply same bias correction to future climate simulations

Assumption: Biases in the future will be similar to that in the
historical base period

First, we will define our historical period as 1950–1999. We will
...
Recall, an important assumption is that biases identified in the
historical period will represent those in future projected periods.

|image36|

Let's use ESM model projection data for December monthly
precipitation in a 2° X 2° gridbox in the central part of the U.S. state
of California.

|image37|

Here we see the ESM historical simulations (the light red lines)
...
There are 41 unique historical simulations, representing the 16
...
If we look at the CDF for just the observations, we see this
distribution of values for 50 Decembers.

Looking at this CDF, imagine a corresponding times series of
observed December precipitation for 1950–1999.

What would be the value of the highest peak? The highest peak is
about 23 inches, (˜580 mm).

What would be the lowest value? That is the X axis value where
...
The red trace shows the CDF of climate model projection #4, one
...
Recall the earlier section on deriving bias adjustments. Compare
these two CDFs. The following bias or biases are indicated with the
Projection #4 for the historical period:

-  The simulated range of values is too narrow.

-  There is a wet bias for low values.

-  There is a dry bias for high values.
...
In this graphic, once again we see the CDF plots in bold red and
...
Although there are a variety of simulations, most individual
...
In this time series from Oklahoma and Texas, we see an ensemble
...
Bias correction when data shows a trend with time

-  Distinguish between errors and trends

-  Remove trend temporarily to adjust for bias

If there is a trend, we must distinguish this from the model
...
In this hypothetical case, we see a temperature simulation that
...
We start by quantifying the linear temperature trend in the
simulation and then removing it.

|image46|

This detrending is done before bias correction.

|image47|

The bias correction is then done on the detrended values.

|image48|

Once the bias correction is complete, the linear trend is added
back.

|image49|

The final result is a time series into the future that has been
corrected for bias and still contains the trends from the model
projections.

|image50|

Data trends are not the only reason that future values may lie
...
-  Downscale global projections to 1/8° grid from 2° grid

-  Using 1/8° observed climatology from historical period

This step will use the bias-corrected coarse 2° x 2° data and
...
Let's review the basic statistical downscaling procedure that was
illustrated in the exercise shown earlier in this case study, but this
time with real BCSD data.

|image51|

First we determine how the coarse bias-corrected projection data
differ from observed climatology "coarsened" to that same 2° spatial
resolution.

We do this by computing a conversion "factor". The "factor" will
be:

-  Incremental for temperature: that is, the temperature factor
   equals the coarse resolution temperature simulation minus the coarse
   resolution observed temperature climatology

-  Multiplicative for precipitation: that is, the precipitation
...
Next we downscale the "factor" values from the 2° grid to the
1/8° spatial grid using an interpolation technique.

|image53|

Finally we merge the 1/8° "factor" information with the 1/8°
observation-based climatology to compute the high spatial resolution
projection data so that:

-  High resolution temperature simulation equals the observed
   high resolution temperature climatology plus the high resolution
   temperature factor, and

-  High resolution precipitation simulation equals the observed
   high resolution precipitation climatology times the high resolution
   precipitation factor.

|image54|

Let's look at an example of how this method is applied with mean
monthly precipitation rate, expressed in units of mm/day.

Here is the 2° spatial grid of observation-based climatology, and
...
The result is a conversion factor map on a 2° grid. These factors
...
The factor values are next downscaled to a spatial resolution of
1/8° using an inverse distance squared method, in this case, the SYMAP
method.

|image56|

Next we bring back the high-resolution, 1/8°, observed
...
We have now spatially downscaled the simulated data so that the
simulation means are retained and the observed variances are preserved.

Summary of BCSD:

-  Data reproduced to make it more representative of local
   climate forcings

   -  Bias correction to remove systematic error

   -  Downscaling to obtain finer spatial resolution

Let's take a moment to summarize what we have done to this point.
...
In many cases we will be using an ensemble of climate projections
...
1) a temperature ensemble, and 2) a precipitation ensemble. The
...
In this time-series view we can see sequencing characteristics,
...
In an ensemble of climate model projections, each projection
...
-  Useful for statistical representations

-  Not useful for detailed sequencing

The strength of climate projections is to capture trends. Climate
...
An ensemble of projections is better at smoothing out the short
...
There are several methods for re-introducing sequencing into the
...
1. Bias correction of simulations

2. Statistical downscaling of the simulation data to obtain more
   regionally representative information

3. Use of an ensemble of projections for more confidence in
   simulated trends

With that in mind, we will now analyze and compare three
projection ensembles that show these steps in BCSD.

|image60|

Each of these three time series show examples of an ensemble of
...
The first of these three time series shows the data before bias
correction and spatial downscaling and therefore at the raw 2° spatial
resolution.

The black line shows the observed data through 1999. The blue and
...
The light red lines show the full set of ensemble traces.

|image62|

The next one shows the data after bias correction but still at
the coarse 2° spatial resolution.

|image63|

And the last one, which we have already seen, shows the data
after both bias correction and downscaling to the 1/8° spatial
resolution.

The light blue lines in this case show the full set of ensemble
traces.
...
In STEP 3 we will characterize trends and variability of the
future climate. With these data we will:

-  Define climate change scenarios described by future
   temperature and precipitation, and then

-  Identify specific climate model projections that will inform
   each climate change scenario

We will examine the future trends and variability as represented
by all of the ESM projections taken together, to determine a set of
climate change scenarios to work with.

Recall that a climate change scenario describes general
...
1. The central tendency, which is based on the median value of
   the projections, and used to establish whether a general trend is
   depicted by the data, and

2. The spread, which is the variability around the general trend
   in the projection data.

Next, we will identify specific ESM projections out of the full
...
Approaches for expressing future time periods:

-  Period change approach

-  Transient approach

The example we show will use the *period change* approach for
...
The outcome of this section will be a set of climate change
...
Let's use a simple example to illustrate why it is useful to
...
From our "central tendency" values we can derive a warming trend
...
Of course climate model projections don't have such exact
...
The 50-year trend continues to show that the central tendency of
...
We will come back to central tendency and spread in a moment. But
...
Now let's say we have a total of 5 ESM projections to simulate
...
Change between historical and future periods is basis for period
change method

The change between these two periods is the basis for what we
...
The simulated future mean temperatures are labeled for
...
To make this clear, let's connect the projection-specific lines.
...
-  Projection 2 produced the largest warming (+5.0degC) in the
...
This exercise showed how central tendency and spread are derived
...
Two sets of climate change data:
...
Bias corrected and spatially downscaled using BCSD

|image72|

Each **annual** set contains **24** separate subsets of values,
one for each month. And each subset will have 112 (for AR4) simulated
values, one for each of the projections.

|image73|

So there are actually 24 subsets of 112 monthly precipitation and
...
To answer that question, let's take a moment to recall the
...
Monthly time steps are more applicable for resolving seasonal
...
Annual time steps, however, are likely to be sufficient and may
...
-  Sufficient for detecting long-term trends

-  Commonly used in period change studies

|image75|

Looking at the example we just completed, projection #5 was
...
-  Annual data is representative of monthly change

-  Projection #5 will represent central tendency of each monthly
   simulation

Going forward with real data examples, we will be representing
...
Let's go to a climate study area in southwestern Oklahoma and
...
In this temperature times series, we see the annual means of the
...
The general tendency in the temperature with time shows the
following:

-  The ensemble median line shows a trend to warmer temperatures
   in the future.

-  There is an increase in the range of possibilities, or spread,
   with time.
...
The precipitation projection time series does not show a distinct
...
The time series plots for this location show a likely warmer
...
The time series plots of mean annual temperature and mean annual
...
-  Often used in impacts studies

-  Evaluate changes between specified historical and specified
   future periods

|image81|

Here we see the two periods of analysis that will be used for our
period change evaluation: the historical base period of 1970–1999 and
the future period of 2040–2069.

|image82|

The horizontal red and blue lines represent the 30-year means for
...
-  Historical period: 41 unique simulation means

-  Future projections: 112 unique simulation means

-  112 unique change traces between historical and future
   periods

|image83|

The result is 112 unique change traces between the historical and
future periods.

Just as we learned in the illustration that we did earlier, the
...
-  Slope of the lines: magnitude and sign of period change
...
We will now take a look at the BCSD simulation data from our
...
These time series are stretched a bit more in the vertical with
...
The projection that represents the median change, or the 50th
...
Let's come back to our time series with the simulation means and
...
For annual precipitation, the determination of the spread limits
...
To deal with the uncertainty indicated by future simulations, we
...
**Period Change: Central Tendency and Spread for Mean Annual
...
From the period changes indicated by the climate projection data,
...
We now have enough information to do this. So, next we will
define our climate change scenarios using the central tendency and
spread for both temperature and precipitation.

Because all of the temperature simulations showed a warming
...
As a result we have five climate change scenario definitions:
...
4. Hotter and drier, and

5. Central tendency which is warmer with similar precipitation.

Identify Specific Projections to Inform Climate Change Scenarios
''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''

Now that we have five climate change scenarios based on
...
Let's take a moment to make sure it is clear where we are in the
...
**Combine temperature and precipitation change information**

We will now construct a figure that allows us to combine the
...
Now let's extend the central tendency of the temperature change
...
The 10th and 90th percentile lines that define the spread limits
...
Each specific projection is represented by a dot on the scatter
...
The individual projection run numbers on this plot are each
...
It may strike you as odd that the two "hotter" scenarios come
...
Recall that the A2 and A1B scenarios don't diverge until after
...
Using a single projection as input to each climate change
...
To do this, we start with the five single projections identified
...
The result is that we have identified 5 clusters of 10
...
What are the advantages of using an ensemble of multiple
...
Advantages of using an ensemble of projections:

-  Uncertainty in individual projection may mask climate trend
   signal

-  Ensemble of projections may make climate trend signal easier
   to detect
...
The "period change" approach shown in this section is not the
...
-  Transient method preserves non-linear trends

-  Period change method sufficient for many assessment questions

Using a period change approach will mask the non-linearity. In
this case it hides the rate of increase in temperature in future
periods.

The transient method preserves the non-linear trends and
therefore may provide more useful information when changes in rates are
important.

The choice of which method to use, period change or transient,
...
As ESMs become more sophisticated, they may reach a point where
...
As ESMs become more sophisticated:

-  Future simulations should be less dependent on historical
   data
...
-  Period change approach used to examine full set of climate
   projection data

-  Five climate change scenarios developed

-  Specific projections selected to inform each scenario

   -  Single projection per scenario

   -  Ensemble of projections per scenario

Review:

-  The term "central tendency", as defined in this module in the
...
-  The period change method differs from the transient method in
...
-  A possible advantage of using an ensemble of projections
...
-  The spread used to determine the range of possible outcomes
...
-  In addition to the central tendency, the additional four
   climate change scenarios that we are working with are: hotter/wetter,
   warmer/wetter, warmer/drier, hotter/drier.

In Step 4 we will develop the five climate change scenarios in
...
In STEP 4 we are now at the point where we will develop
...
These inputs which we call "climate-adjusted weather" are
...
We will develop these inputs by merging historical observations
with trends from the specific ESM projections that we selected to
represent the five climate change scenarios.

Recall that those scenarios, used to plan for a range of possible
outcomes are:
...
5. Central tendency: Warmer with similar mean annual
   precipitation.

|image101|

We will show two approaches for generating climate change time
...
But how can we get realistic detail on short time steps from
future simulations?

We will demonstrate one approach that has been called the "as if"
...
Colorado River Water Availability Study, Colorado Water
...
In this approach we will start with the observed past weather
conditions and alter those "as if" the future climate changes had taken
place already.

"As if" approach:

-  Estimate details of future conditions

-  Alter observed past weather as if climate change had already
   occurred

|image102|

Important assumption: Historical observations are representative
of future variability and sequences

The outcome is to develop climate-adjusted weather inputs that
...
"Climate-adjusted weather" inputs:

-  Contain observation-based details
...
Two important terms that we will be using are: climate-adjusted
...
Sequencing describes the order of events. For example, do wet and
...
-  Climate-adjusted weather: Detailed inputs for future time
   period
   ...
   -  Use the "as if" approach: Alter past observations with
      future trends

-  Sequencing: The order of climate-adjusted weather events

**Why Not Use Data Right from the Climate Projections?**
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

The climate change projections that we already identified are
...
AR4 climate models do actually simulate weather in order to more
...
Let's consider a hypothetical situation where over the course of
...
Now let's look at simulated December monthly precipitation for
...
But, as you can see, the time series plots indicate that the
...
**Develop Monthly Climate Weather: Hypothetical Case**
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now we will walk through a method for generating climate-adjusted
...
As introduced earlier, our approach involves taking the details
...
Step 4: We then quantify the difference in the means between the
...
Step 5: Finally we apply the change factor to the
...
-  Change Factor: how we convert observed conditions to
   conditions that are reasonable to expect in a future climate

In this methodology, step 4 involves the computation of a change
...
To get a better feel for what the change factor is representing,
...
Now let's consider a climate projection, in red, where simulated
...
Now we can bring back the observation-based December data and
...
This time series of potential future climate-adjusted weather
...
When climate-adjusted weather is shown for all months (each with
...
Consider this two-year slice of a future period climate-adjusted
...
We can then add the Februarys, with a February-specific change
factor for the same hotter/wetter scenario.

|image118|

...then the Marches, with the March-specific change factor, and
so on.
...
In the end we will have a monthly time series for this specific
climate change scenario.

**Develop Monthly Climate Weather: Real Data**
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

|image121|

Change factor: based on difference in 30-year monthly means
between

-  Historical period, 1970-1999

-  Future period, 2030-2059

Using real data, we will now demonstrate this methodology for the
...
-  Repeated for the other 11 months

-  Repeated for mean monthly temperature

So we will produce 10 sets of climate weather inputs for December
...
We will compare the simulated historical period values with the
...
Here is a time series plot of observed monthly precipitation
...
Let's make sure we understand the change factor process. The
...
Here we add the same information for the month of December in the
...
Here we see the simulation-based CDFs of the 1970–1999 historical
...
The CDFs of the 5 scenarios are represented by:

-  Red, for hotter/drier,

-  Green, for hotter/wetter,

-  Yellow, for warmer/drier,

-  Blue, for warmer/wetter, and

-  Gray, for the central tendency scenario

Looking first at the top graph showing the single-projection
...
It is difficult to differentiate the different scenarios because
...
We see more differences between the five climate change scenarios
...
Note that the hotter/drier climate change scenario for the Delta
...
Note that we see a greater departure from the observed values in
the Delta method results.

|image126|

Before we get to the change factors, let's remind ourselves of
the magnitude of annual changes in our selected climate projections.

In the Delta method we use simulations of historical and future
...
Likewise, these were the five ensembles of 10 projections for the
five climate change scenarios that we will use in the Ensemble-informed
Delta method.

Keeping with our hotter and wetter scenario as an example, the 10
...
Remember that these are all annual changes. Any specific month
...
-  Based on differences between simulated historical and future
   values

Now lets take a closer look at how the change factor is derived.
...
If we compare the historical and the future simulations, we can
...
Let's focus on just one scenario, hotter and wetter, represented
by the green traces, shown here for the future and historical
simulations.

|image130|

Next we overlay the 30-year mean for the historical simulation in
...
We will do the same thing using the Ensemble-informed Delta for
...
Again we will take just the green lines, the hotter/wetter
...
There is a greater difference in the means associated with the
...
In the Delta and Ensemble-informed Delta methods there is one
...
Here we are looking at a depiction of the change factors for each
...
The change factors are 1.00 for the central change scenario shown
...
Here we see that the Delta and Ensemble-informed Delta methods
produce different results when exploring how December precipitation is
projected to change in the future.

You can see that the Ensemble-informed Delta method has a more
...
Next we will use the change factors we just derived to compute
...
The December 1951 precipitation is 20 inches. The Delta method,
...
Once we apply change factors associated with all five scenarios
...
Ensemble-informed Delta:

-  Each change factor is an average of change factors from 10
   projections

-  Mutes large variations often seen when using change factors
   based on single projections

As pointed out earlier, in the Ensemble-informed Delta method,
...
Change factors will be computed separately for each month and
...
The first set of five month-specific change factors corresponds
...
Note that the hotter, drier scenario, red, shows a wetter trend
for August and December, even though it is a drier scenario overall.

The second set of five month-specific change factors corresponds
...
We can see that when we use Ensemble-informed Delta there is less
...
Overall, we can see that the green and blue lines, representing
...
The example that we have been using was for the climate-adjusted
...
-  Temperature change factor = future simulation mean –
   historical simulation mean

-  Climate-adjusted temperature inputs = temperature observations
   + temperature change factor

It is derived by subtracting the historical simulation mean from
...
Climate impacts models often require data on daily time steps.
...
These are required to produce estimates of evapotranspiration,
infiltration, snowmelt, runoff, and other hydrologic variables.

Daily climate-adjusted weather:

-  Reflect realistic range of values and sequence of events

-  Based on observations with projected future trends

As with the monthly climate-adjusted weather inputs, we want the
...
Let's consider the daily observed weather in the month of January
...
The temperature change factor is an incremental factor, for
example, +2°. It is applied equally to both daily maximum and minimum
temperatures.

**Exercise: Adjusted daily conditions**
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

In this exercise we will practice deriving daily climate-adjusted
weather values.
...
.. [2]
   Impacts models are designed to simulate the impact of climate
